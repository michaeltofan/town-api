...
/**
 * Attach the account's linked civic actor to the given community so participant
 * confirmation can succeed. Never touches the controlled test actor
...
export async function activatePasskeyAccountAndLinkCommunity(input: {
  ...
  const { actorId } = await setLinkedCivicActorCommunity(input.app, {
    accountId: registration.accountId,
