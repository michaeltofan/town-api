...
describe('loadEnv', () => {
  ...
  it('defaults PASSWORD_SIGN_IN_ENABLED to false and parses true independently', () => {
    const disabled = loadEnv({
      DATABASE_URL: 'postgres://town:town@127.0.0.1:5432/town',
    });
    expect(disabled.PASSWORD_SIGN_IN_ENABLED).toBe(false);
    ...
    expect(enabled.PASSWORD_SIGN_IN_ENABLED).toBe(true);
  ...
  it('requires session and rate-limit keys when PASSWORD_SIGN_IN_ENABLED is true', () => {
    expect(() =>
      loadEnv({
        DATABASE_URL: 'postgres://town:town@127.0.0.1:5432/town',
        PASSWORD_SIGN_IN_ENABLED: 'true',
  ...
  it('requires WEBAUTHN_ALLOWED_ORIGINS when PASSWORD_SIGN_IN_ENABLED is true', () => {
    expect(() =>
      loadEnv({
        DATABASE_URL: 'postgres://town:town@127.0.0.1:5432/town',
        PASSWORD_SIGN_IN_ENABLED: 'true',
