...
describe("canonical foundation content lock", () => {
  ...
  it("locks Italian, German, and Romanian signal identity and approved copy fields", () => {
    ...
    expect(milanoOne?.summary).toContain(
      "Le radici hanno sollevato il marciapiede",
    );
  ...
  it("locks the Romanian city-expansion communities and signal identity", () => {
    const clujCommunity = FOUNDATION_COMMUNITIES.find(
      (c) => c.slug === "cluj-napoca-ro",
    );
    const sibiuCommunity = FOUNDATION_COMMUNITIES.find(
      (c) => c.slug === "sibiu-ro",
    );
    const iasiCommunity = FOUNDATION_COMMUNITIES.find(
      (c) => c.slug === "iasi-ro",
    );
    const timisoaraCommunity = FOUNDATION_COMMUNITIES.find(
      (c) => c.slug === "timisoara-ro",
    );
  ...
  it("locks the German/Austrian city-expansion communities and signal identity", () => {
    const kolnCommunity = FOUNDATION_COMMUNITIES.find(
      (c) => c.slug === "koln-de",
    );
    const dortmundCommunity = FOUNDATION_COMMUNITIES.find(
      (c) => c.slug === "dortmund-de",
    );
    const stuttgartCommunity = FOUNDATION_COMMUNITIES.find(
      (c) => c.slug === "stuttgart-de",
    );
    const frankfurtCommunity = FOUNDATION_COMMUNITIES.find(
      (c) => c.slug === "frankfurt-de",
    );
    const salzburgCommunity = FOUNDATION_COMMUNITIES.find(
      (c) => c.slug === "salzburg-at",
    );
  ...
  it("locks the French and Hungarian foundation communities and locales", () => {
    expect(
      FOUNDATION_COMMUNITIES.slice(-5).map((community) => community.slug),
    ).toEqual([
      "marseille-fr",
      "lyon-fr",
      "toulouse-fr",
    ...
    expect(
      FOUNDATION_SIGNALS.filter((signal) => signal.locale === "fr-FR"),
    ).toHaveLength(9);
    expect(
      FOUNDATION_SIGNALS.filter((signal) => signal.locale === "hu-HU"),
    ).toHaveLength(6);
    expect(FOUNDATION_SIGNAL_IDS.marseilleSignal1).toBe(
      "00000000-0000-4000-8000-000000001401",
    );
    expect(FOUNDATION_SIGNAL_IDS.szegedSignal3).toBe(
